...
# Import Libraries
import streamlit as st                  # Used to deploy on Streamlit
import os                               # Used to get API key from .env
import json                             # Used to transmit text in web environment
import chromadb                         # Used for RAG database with ChromaDB
from pathlib import Path                # Used for path loading of files
from dotenv import load_dotenv          # Used to create and load .env file
from openai import OpenAI               # Used for OpenAI API commands

# Import Custom Modules
from risk_rules import calculate_business_risk          # Custom business rules
from file_utils import read_uploaded_file               # Custom read uploaded file function
from pdf_utils import create_procurement_report_pdf     # Custom PDF creation function
from rag_utils import (                                 # Custom RAG functions
    retrieve_chunks_from_chroma,
    generate_rag_answer,
    format_retrieved_sources,
    index_uploaded_pdf_in_chroma
)

# Load environment variables and initialize OpenAI client
load_dotenv()
openai_client = OpenAI(api_key=os.getenv("OPENAI_API_KEY"))    # GPT + embeddings

# Set up persistent ChromaDB storage for the RAG knowledge base
base_dir = Path(__file__).parent
chroma_db_path = base_dir / "chroma_db"

chroma_client = chromadb.PersistentClient(path=str(chroma_db_path))                     # Chroma database management
rag_collection = chroma_client.get_or_create_collection(name="procurement_documents")   # Vector collection
    ...
    st.markdown(
        ...
        <div class="sidebar-info">
            ...
            <ul>
                ...
                <li><strong>Ask Knowledge Base</strong> — build a document knowledge base and ask grounded questions using RAG.</li>
            </ul>
        </div>
        ''',
        unsafe_allow_html=True
        ...
        if generate_clicked:
            ...
            try:
                ...
                with st.spinner(
                    ...
                    extraction_response = openai_client.responses.create(
                        model="gpt-4.1-mini",
        ...
        if generate_report_clicked:
            ...
            try:
                ...
                with st.spinner("Generating procurement report..."):
                    report_response = openai_client.responses.create(
                        model="gpt-4.1-mini",
                        input=report_prompt
                    )
...
# ============================================================
if analysis_mode == "Ask Knowledge Base":

    st.markdown(
        '<p class="section-label">🧠 Procurement Knowledge Base</p>',
        unsafe_allow_html=True
    )
    ...
    # Button to initiate chromaDB indexing
    ...
    if build_kb_clicked:
        ...
        # Proceed if uploaded files exist
        else:

            try:

                # Attempt to index the uploaded PDF files
                with st.spinner("Building knowledge base..."):

                    chroma_client.delete_collection(name="procurement_documents")                           # Empty collection if user uploads files and clicks button
                    rag_collection = chroma_client.get_or_create_collection(name="procurement_documents")   # Create new collection with uploaded files

                    for uploaded_file in uploaded_rag_files:        # For-loop for indexing uploaded files
                        index_uploaded_pdf_in_chroma(
                            uploaded_file,
                            rag_collection,
                            openai_client
                ...
                st.success(
                    f"Knowledge base built successfully from "
                    f"{len(uploaded_rag_files)} document(s)."
                )

            # Error message in case indexing failed
            except Exception as error:
                st.error(
                    f"Unable to build the knowledge base: {error}"
                )
    
    # Headline for text input field
    st.markdown(
        '<p class="section-label">💬 Ask the Knowledge Base</p>',
        unsafe_allow_html=True
    )
    ...
    question = st.text_input(
        ...
        placeholder="e.g. Which supplier offers the shorter delivery time?"
    )
    ...
    # Check if question button was clicked
    if ask_clicked:

        # Error message if button was clicked without a question
        if not question.strip():
            st.warning(
                "Please enter a question."
            )

        # Error message if there are no indexed PDF files
        elif rag_collection.count() == 0:
            st.warning(
                "The knowledge base is empty. Please upload and index documents first."
            )

        # Proceed if question and knowledge base exist
        else:
            ...
            try:
                ...
                with st.spinner("Searching knowledge base..."):
                    ...
                    # Get top 3 relevant chunks from chromaDB
                    ...
                    # Format retrieved source information
                    sources = format_retrieved_sources(
                        retrieved_chunks
                    )

                # Headline for answer text box
                st.markdown(
                ...
                # Only display sources if needed information was inside the uploaded files
                if answer.strip() != "I cannot answer this question based on the provided documents.":

                    # Headline for sources text box
                    st.markdown(
            ...
            # Error message if generating an answer failed
            except Exception as error:
                st.error(
